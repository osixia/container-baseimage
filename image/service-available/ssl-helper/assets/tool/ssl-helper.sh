...
if [ ! -e $SSL_CRT ] || [ ! -e $SSL_KEY ]; then
  ...
  # Get ssl vars
  source /osixia/service-available/ssl-helper/assets/tool/get-ssl-env-var.sh
  ...
  # GNUTLS
    ...
    if [ -z "$SSL_CA_CRT" ]; then
      ...
      source /osixia/service-available/ssl-helper-gnutls/assets/tool/create-gnutls-crt-file-infos.sh
