...
A Debian Experimental based docker image to help you build reliable image quickly. This image provide a simple opinionated solution to build multiple or single process image with minimum of layers and an optimised build.
...
In the Dockerfile we are going to:
  ...
  - Define ports exposed and volumes if needed.
       ...
       FROM osixia/experimental-light-baseimage:0.1.1
...
Here simple Dockerfile example how to add a service-available to an image:
        ...
        # https://github.com/osixia/docker-light-baseimage
        FROM osixia/experimental-light-baseimage:0.2.1
        MAINTAINER Your Name <your@name.com>
...
*Run tool* takes several options, to list them:
    ...
    docker run osixia/experimental-light-baseimage:0.2.1 --help
...
Example:
    ...
    docker run -it osixia/experimental-light-baseimage:0.2.1
...
Can also be set by command line converted in python or json:
    ...
    docker run -it --env FRUITS="#PYTHON2BASH:['orange','apple']" osixia/experimental-light-baseimage:0.2.1 printenv
    docker run -it --env FRUITS="#JSON2BASH:[\"orange\",\"apple\"]" osixia/experimental-light-baseimage:0.2.1 printenv
