...
Latest release: 1.0.1 (debian stretch) - 1.0.0 (debian jessie) [Changelog](CHANGELOG.md)
...
In the Dockerfile we are going to:
  ...
  - Define ports exposed and volumes if needed.
        ...
        FROM osixia/light-baseimage:1.0.1
...
*Run tool* takes several options, to list them:
    ...
    docker run osixia/light-baseimage:1.0.1 --help
...
Example:
    ...
    docker run -it osixia/light-baseimage:1.0.1
...
Can also be set by command line converted in python or json:
    ...
    docker run -it --env FRUITS="#PYTHON2BASH:['orange','apple']" osixia/light-baseimage:1.0.1 printenv
    docker run -it --env FRUITS="#JSON2BASH:[\"orange\",\"apple\"]" osixia/light-baseimage:1.0.1 printenv
