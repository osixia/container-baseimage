...
func (e *Exec) Wait() error {
	...
	if e.Cmd == nil {
		return errors.New("Wait called with a nil cmd ")
	}
...
func (e *Exec) waitOrStop(ctx context.Context, cmd *exec.Cmd, interrupt os.Signal, killDelay time.Duration) error {
	...
	if cmd.Process == nil {
		return errors.New("waitOrStop called with a nil cmd.Process — missing Start call?")
	}

	if interrupt == nil {
		return errors.New("waitOrStop requires a non-nil interrupt signal")
	}
