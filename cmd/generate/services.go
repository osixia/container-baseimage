...
var servicesCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		files := helpers.MustVal(core.Instance().Generator().GenerateServices(&servicesCmdFlags.GenerateServicesOptions))
...
func init() {
	...
	cmdlog.AddFlags(servicesCmd.Flags())
}

func addServicesFlags(fs *pflag.FlagSet, gopt *core.GenerateServicesOptions) {
	fs.IntVarP(&gopt.Priority, "priority", "p", config.CoreServicesConfig.DefaultPriority, "services priority")
