...
func AddFlags(fs *pflag.FlagSet) {
	fs.StringP(LevelFlag, "l", log.Levels[log.DefaultLevel], fmt.Sprintf("set log level, choices: %v", strings.Join(log.LevelsList(), ", ")))
	fs.Bool(QuietFlag, false, "show errors only")
	fs.StringP(FormatFlag, "o", string(log.DefaultFormat), fmt.Sprintf("set log format, choices: %v", strings.Join(log.FormatsList(), ", ")))
}

func HandleFlags(cmd *cobra.Command) error {

	if cmd.Flags().Changed(LevelFlag) {
