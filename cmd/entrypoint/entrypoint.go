...
var EntrypointCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		if entrypointCmdFlags.runCmdOnly && !cmd.Flags().Changed(cmdlog.LevelFlag) {
...
func init() {
	...
	EntrypointCmd.Flags().BoolVarP(&entrypointCmdFlags.runCmdOnly, "cmd", "c", false, "run only the given command (skips services, log level defaults to error)")
	EntrypointCmd.Flags().BoolVarP(&entrypointCmdFlags.RunBash, "bash", "b", false, "run Bash alongside other services or command\n")
	EntrypointCmd.MarkFlagsMutuallyExclusive("exec", "cmd")
	...
	cmdlog.AddFlags(EntrypointCmd.Flags())
}
