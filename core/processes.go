...
type Processes interface {
	...
	Start(ps []Process, timeout time.Duration) error
	Stop(ps []Process, timeout time.Duration) error
...
func (prcs *processes) List(opts ...ProcessesListOption) ([]Process, error) {
	...
	if prcslo.TagPrefixInNames != "" {
		...
		for _, name := range prcslo.Names {
			if tag, ok := strings.CutPrefix(name, prcslo.TagPrefixInNames); ok {
	...
	if prcslo.Names != nil {
		...
		for _, file := range files {
			...
			if file.IsDir() {
				log.Debugf("Ignoring directory %v", file)
				continue
			}
			...
			// match pid prefix
			if n, ok := strings.CutSuffix(fn, prcs.config.PIDFileSuffix); ok {
				name = n
			} else if n, ok := strings.CutSuffix(fn, prcs.config.WantedDownFileSuffix); ok {
				name = n
			} else {
				log.Debugf("Ignoring file %v: not matching %v or %v files suffix", file, prcs.config.PIDFileSuffix, prcs.config.WantedDownFileSuffix)
				continue
			}
...
func (prcs *processes) Start(ps []Process, timeout time.Duration) error {
	...
	for _, p := range ps {
		...
		for !p.IsUp() {
			if !deadline.IsZero() && time.Now().After(deadline) {
				return fmt.Errorf("%v: timed out waiting for process to start", p.Name())
			}
...
func (prcs *processes) Stop(ps []Process, timeout time.Duration) error {
	...
	for _, p := range ps {
		...
		for p.IsUp() {
			if !deadline.IsZero() && time.Now().After(deadline) {
				return fmt.Errorf("%v: timed out waiting for process to stop", p.Name())
			}
...
func (prcs *processes) CleanupPIDFiles() {

	log.Trace("processes.CleanupPIDFiles called")
	...
	if err != nil {
		if !os.IsNotExist(err) {
			log.Warningf("Error reading processes directory %v: %v", processesDir, err)
		}
		return
	}
	...
	for _, file := range files {
		...
		fn := file.Name()
		name, ok := strings.CutSuffix(fn, prcs.config.PIDFileSuffix)
		if !ok || name == "" {
			continue
		}

		pidFile := filepath.Join(processesDir, fn)

		data, err := os.ReadFile(pidFile)
		if err != nil {
			log.Warningf("Error reading pid file %v: %v", pidFile, err)
			continue
		}

		pid, err := strconv.Atoi(strings.TrimSpace(string(data)))
		if err != nil {
			log.Warningf("Removing malformed pid file %v: %v", pidFile, err)
			if err := helpers.Remove(pidFile); err != nil {
				log.Warningf("Error removing malformed pid file %v: %v", pidFile, err)
			}
			continue
		}

		// Signal 0 checks process existence without sending an actual signal.
		// nil  – process exists and we can signal it: skip.
		// EPERM – process exists but we lack permission to signal it: skip.
		// ESRCH – process does not exist: pid file is stale, remove it.
		// anything else – unknown error, skip conservatively.
		switch err := syscall.Kill(pid, 0); err {
		case nil:
			log.Debugf("pid file %v: process %v is still running, skipping", pidFile, pid)
			continue
		case syscall.EPERM:
			log.Debugf("pid file %v: process %v exists (EPERM), skipping", pidFile, pid)
			continue
		case syscall.ESRCH:
			// process is gone, fall through to remove the stale pid file
		default:
			log.Warningf("Unexpected error checking pid %v from file %v: %v, skipping", pid, pidFile, err)
			continue
		}

		log.Warningf("Removing stale pid file %v (process %v is not running)", pidFile, pid)
		if err := helpers.Remove(pidFile); err != nil {
			log.Warningf("Error removing stale pid file %v: %v", pidFile, err)
		}
	}
}
