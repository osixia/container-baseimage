...
type LifecycleOptions struct {
	...
	KillAllOnExit        bool
	KillAllOnExitTimeout time.Duration
...
func (lc *lifecycle) exec(ctx context.Context) *helpers.Exec {
	return helpers.NewExec(ctx).WithTimeout(lc.options.KillAllOnExitTimeout)
}
...
func (lc *lifecycle) run(ctx context.Context) {
	...
	go func() {
		...
		for pids != nil || errs != nil {
			select {
			case <-done:
				return
			case pid, ok := <-pids:
				if !ok { pids = nil; continue }
				log.Debugf("Reaped pid: %v", pid)
			case e, ok := <-errs:
				if !ok { errs = nil; continue }
...
func (lc *lifecycle) killAll() {
	...
	timeout := lc.options.KillAllOnExitTimeout
