...
func WithoutServicesNames(names []string) ServicesListOption {
	return func(svcslo *ServicesListOptions) {
		if svcslo.ExcludedNames == nil {
			svcslo.ExcludedNames = []string{}
		}
		svcslo.ExcludedNames = append(svcslo.ExcludedNames, names...)
	}
}
...
func WithoutServicesTags(tags []string) ServicesListOption {
	return func(svcslo *ServicesListOptions) {
		if svcslo.ExcludedTags == nil {
			svcslo.ExcludedTags = []string{}
		}
		svcslo.ExcludedTags = append(svcslo.ExcludedTags, tags...)
	}
}
...
func (svcs *services) List(opts ...ServicesListOption) ([]Service, error) {
	...
	if (svcslo.ExcludedNames != nil && len(svcslo.ExcludedNames) == 0) || (svcslo.ExcludedTags != nil && len(svcslo.ExcludedTags) == 0) {
		log.Trace("empty excluded names or tags")
		return nil, nil
	}
	...
	if svcslo.TagPrefixInNames != "" {
		...
		for _, name := range svcslo.Names {
			if tag, ok := strings.CutPrefix(name, svcslo.TagPrefixInNames); ok {
				tags = append(tags, tag)
			}
		}
		log.Tracef("tags found: %v", tags)

		log.Tracef("search excluded tags in excluded name with prefix: \"%v\"", svcslo.TagPrefixInNames)
		var excludedTags []string
		for _, name := range svcslo.ExcludedNames {
			if tag, ok := strings.CutPrefix(name, svcslo.TagPrefixInNames); ok {
				excludedTags = append(excludedTags, tag)
			}
		}
		log.Tracef("excluded tags found: %v", tags)

		// search services with or without those tags
		tss, err := svcs.List(WithServicesTags(tags), WithoutServicesTags(excludedTags))
		if err != nil {
			return nil, err
		}
	...
	for _, c := range cs {
		...
		if slices.Contains(svcslo.ExcludedNames, c.Name()) {
			continue
		}
		...
		if svcslo.ExcludedTags != nil && c.HasTag(svcslo.ExcludedTags...) {
			continue
		}
