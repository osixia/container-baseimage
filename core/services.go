...
func (svcs *services) List(opts ...ServicesListOption) ([]Service, error) {
	...
	if svcslo.TagPrefixInNames != "" {
		...
		log.Tracef("excluded tags found: %v", excludedTags)
	...
	if svcslo.Names != nil {
		...
		for _, subDir := range subdirs {
			...
			if !subDir.IsDir() {
				log.Debugf("Ignoring %v: not a directory", subDir)
				continue
			}
...
func (s *service) Require() error {
	...
	return helpers.Remove(s.optionalFile)
}
...
func (s *service) Unlink() error {
	...
	return helpers.Remove(s.linkedFile)
}
