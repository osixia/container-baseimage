...
func (gen *generator) GenerateDockerfile(gopt *GenerateDockerfileOptions) ([]string, error) {
	...
	fromImage := gen.config.fromImage
	if gopt.FromImage != "" {
		fromImage = gopt.FromImage
	}
...
func (gen *generator) restoreEnv() {
	...
	for _, e := range gen.envBackup {
		...
		if err := os.Setenv(kv[0], kv[1]); err != nil {
			log.Warningf("Failed to set environment variable %v=%v", kv[0], kv[1])
		}
	}
}
...
func (gen *generator) setEnv(kv map[string]string) {
	...
	for k, v := range kv {
		if err := os.Setenv(k, v); err != nil {
			log.Warningf("Failed to set environment variable %v=%v", k, v)
		}
	}
}
