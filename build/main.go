...
func main() {
	...
	job.ExportJob = func(ctx context.Context, client *dagger.Client, options any) ([]*job.BuildResult, error) {

		o := options.(*job.ExportOptions)

		builds, err := job.DefaultExportJob(ctx, client, o)
		if err != nil {
			return nil, err
		}
	...
	job.BuildArgs = func(options *job.BuildImageOptions, image *config.Image, platform *config.Platform, tag string) []dagger.BuildArg {
		...
		return append(buildArgs, goarchArg)
	}
