...
func buildImage(ctx context.Context, workspace *dagger.Directory, dockerfile string, options *BuildImageOptions, image *config.Image, nonroot bool) (*BuildResult, error) {
	...
	for _, platform := range options.Platforms {
		...
		// add oci labels
		c = c.WithLabel("org.opencontainers.image.title", image.ImageName).
...
func buildArgs(image *config.Image, version string, contributors string, platform *config.Platform, tag string) []dagger.BuildArg {
	...
	if contributors != "" {
		contributorsArg := dagger.BuildArg{
			Name:  "CONTRIBUTORS",
			Value: contributors,
		}
		buildArgs = append(buildArgs, contributorsArg)
	}

	// image build args
	rootImageArg := dagger.BuildArg{
		Name:  "ROOT_IMAGE",
		Value: image.RootImage,
	}

	imageNameArg := dagger.BuildArg{
		Name:  "IMAGE_NAME",
		Value: image.ImageName,
	}

	imageTagArg := dagger.BuildArg{
		Name:  "IMAGE_TAG",
		Value: tag,
	}
	...
	return append(buildArgs, rootImageArg, imageNameArg, imageTagArg, goarchArg)
}
