...
var TagRegex = `^[0-9]+\.[0-9]+\.[0-9]+-?[^.]*$`                   // x.y.z or x.y.z-a with x, y and z numbers and a any character except '.'
var ReleaseTagRegex = `^[0-9]+\.[0-9]+\.[0-9]+(?:-[0-9]+)?$`       //  x.y.z or x.y.z-a with x, y, z and a numbers
var PrereleaseTagRegex = `^[0-9]+\.[0-9]+\.[0-9]+-[a-z][a-z0-9]*$` // x.y.z-a with x,y and z numbers and a that must start with a letter and may contain alphanumeric characters
